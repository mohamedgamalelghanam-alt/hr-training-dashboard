...
# 2. Data Loading Function (Dynamic File Scanner)
@st.cache_data
def load_data():
    files_in_dir = os.listdir(".")
    
    # البحث عن أي ملف ينتهي بـ .csv أو .xlsx ويحتوي على كلمة training
    data_files = [f for f in files_in_dir if f.lower().endswith(('.csv', '.xlsx')) and 'train' in f.lower()]
    
    # لو ملاقاش ملف فيه كلمة train، ياخد أول ملف csv أو xlsx يقابله في الفولدر برة
    if not data_files:
        data_files = [f for f in files_in_dir if f.lower().endswith(('.csv', '.xlsx')) and f != 'requirements.txt']

    if data_files:
        actual_filename = data_files[0]
        # التحقق من نوع الامتداد للقراءة بالطريقة الصحيحة
        ...
        raise FileNotFoundError(f"لم يتم العثور على أي ملف بيانات (CSV أو Excel) في المسار الرئيسي! الملفات المتاحة هي: {files_in_dir}")

try:
    df = load_data()
except Exception as e:
    st.error(f"⚠️ حدث خطأ أثناء تحميل البيانات من السيرفر: {e}")
    st.info("💡 نصيحة مهندس: تأكد من رفع ملف البيانات في المجلد الرئيسي (Root) بجوار ملف app.py مباشرة وليس داخل مجلد فرعي.")
    st.stop()
