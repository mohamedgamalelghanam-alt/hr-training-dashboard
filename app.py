...
# تغليف التطبيق بالكامل لصيد أي خطأ خفي
try:
    # 2. Data Loading Function
    @st.cache_data
    def load_data():
        actual_filename = "Training.xlsx - Sheet1.csv"
    ...
    # Date Filter (تم تحويل التواريخ إلى صيغة date لتفادي مشاكل السيرفرات)
    ...
    st.error("🎯 مهندس محمد، قفشنا الخطأ الخفي برمجياً!")
    st.subheader(f"نوع الخطأ: {main_error}")
    st.markdown("**تفاصيل ومكان السطر المتسبب في المشكلة بالظبط:**")
